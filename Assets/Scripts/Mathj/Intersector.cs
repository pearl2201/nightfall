...
namespace Pika.Base.Mathj
{
    public class Intersector
    {
        ...
        {
            return (int)Math.Sign((linePoint2.x - linePoint1.x) * (point.y - linePoint1.y)
        ...
        {
            return (int)Math.Sign((linePoint2X - linePoint1X) * (pointY - linePoint1Y)
        ...
        {
            Vector2 lastVertice = polygon[polygon.Count() - 1];
            bool oddNodes = false;
            for (int i = 0; i < polygon.Count(); i++)
            {
                Vector2 vertice = polygon[i];
        ...
        */
        ...
        {
            Vector3 lastVertice = polygon[polygon.Count() - 1];
            bool oddNodes = false;
            for (int i = 0; i < polygon.Count(); i++)
            {
                Vector3 vertice = polygon[i];
        ...
        //				return false;
        //			}
        //			s.set(floatArray2.get(floatArray2.Count - 2), floatArray2.get(floatArray2.Count - 1));
        ...
        //						if (floatArray.Count < 2 || floatArray.get(floatArray.Count - 2) != ip.x
        //								|| floatArray.get(floatArray.Count - 1) != ip.y) {
        //							floatArray.add(ip.x);
        //							floatArray.add(ip.y);
        //						}
        //					}
        ...
        public static bool intersectRayTriangles(Ray ray, float[] triangles, Vector3 intersection)
        {
            ...
            if (triangles.Length / 3 % 3 != 0)
                throw new Exception("triangle list size is not a multiple of 3");
        ...
        public static bool intersectRayTriangles(Ray ray, List<Vector3> triangles, Vector3 intersection)
        {
            ...
            {
                bool result = intersectRayTriangle(ray, triangles[i], triangles[i + 1], triangles[i + 2],
                        tmp);
        ...
        public static void splitTriangle(float[] triangle, Plane plane, SplitTriangle split)
        {
            ...
            bool r2 = plane.testPoint(triangle[0 + stride], triangle[1 + stride],
        ...
        public class SplitTriangle
        {
            ...
            {
                return "SplitTriangle [front=" + string.Join(",", front) + ", back=" + string.Join(",", back) + ", numFront="
