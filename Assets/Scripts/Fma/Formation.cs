 ...
 * 
 * @author davebaol */
public class Formation<T> where T : Vector<T>
{
    ...
    private T positionOffset;
    private Matrix3 orientationMatrix = new Matrix3();

    /** The location representing the drift offset for the currently filled slots. */
    private Location<T> driftOffset;
    ...
    public Formation(Location<T> anchor, FormationPattern<T> pattern) : this(anchor, pattern, new FreeSlotAssignmentStrategy<T>(), null)
    {
	 ...
	 * @throws IllegalArgumentException if the anchor point is {@code null} */
    public Formation(Location<T> anchor, FormationPattern<T> pattern, SlotAssignmentStrategy<T> slotAssignmentStrategy) : this(anchor, pattern, slotAssignmentStrategy, null)
    {
    ...
    {
        if (anchor == null) throw new Exception("The anchor point cannot be null");
        this.anchor = anchor;
        this.pattern = pattern;
        this.slotAssignmentStrategy = slotAssignmentStrategy;
        this.motionModerator = motionModerator;

        this.slotAssignments = new List<SlotAssignment<T>>();
    ...
    public bool changePattern(FormationPattern<T> pattern)
    {
    ...
    public bool addMember(FormationMember<T> member)
    {
        // Find out how many slots we have occupied
        int occupiedSlots = slotAssignments.Count();
    ...
    private int findMemberSlot(FormationMember<T> member)
    {
        ...
        {
            if (slotAssignments.ElementAt(i).member == member) return i;
        }
        return -1;
    }
    ...
    {
        return slotAssignments.ElementAt(index);
    }
    ...
    public void updateSlots()
    {
        ...
        for (int i = 0; i < slotAssignments.Count(); i++)
        {
            SlotAssignment<T> slotAssignment = slotAssignments.ElementAt(i);
            ...
            // [17:31] <@Xoppa> davebaol, interface Transform<T> where T : Vector<T> { T getTranslation(); T getScale(); float getRotation();
            ...
            if (relativeLocPosition is Vector2 v2)
                v2.mul(orientationMatrix);
            else if (relativeLocPosition is Vector3 v3)
                v3.mul(orientationMatrix);
