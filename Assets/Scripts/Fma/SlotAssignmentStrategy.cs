 ...
 * 
 * @author davebaol */
public interface SlotAssignmentStrategy<T> where T : Vector<T>
{

    /** Updates the assignment of members to slots */
    void updateSlotAssignments(List<SlotAssignment<T>> assignments);

    /** Calculates the number of slots from the assignment data. */
    int calculateNumberOfSlots(List<SlotAssignment<T>> assignments);

    /** Removes the slot assignment at the specified index. */
    void removeSlotAssignment(List<SlotAssignment<T>> assignments, int index);
