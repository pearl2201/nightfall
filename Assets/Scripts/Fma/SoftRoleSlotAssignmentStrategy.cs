...
// *
// * @author davebaol */
//public class SoftRoleSlotAssignmentStrategy<T : Vector<T>> : BoundedSlotAssignmentStrategy<T> {
//
//	protected SlotCostProvider<T> slotCostProvider;
//
//	protected float costThreshold;
//
//	private List<bool> filledSlots;
//
...
//		List<MemberAndSlots<T>> memberData = new List<>();
//
...
//
//				SlotAssignment<T> slot = assignments.ElementAt(j);
//
...
//		if (numberOfAssignments > filledSlots.Count()) filledSlots.ensureCapacity(numberOfAssignments - filledSlots.Count);
//		filledSlots.Count = numberOfAssignments;
//		for (int i = 0; i < numberOfAssignments; i++)
...
//			MemberAndSlots<T> memberDatum = memberData.ElementAt(i);
//
//			// Choose the first slot in the list that is still empty (non-filled)
//			memberDatum.costAndSlots.sort();
//			int m = memberDatum.costAndSlots.Count;
//			for (int j = 0; j < m; j++) {
//				int slotNumber = memberDatum.costAndSlots.ElementAt(j).slotNumber;
//
//				// Check if this slot is valid
//				if (!filledSlots.ElementAt(slotNumber)) {
//					// Fill this slot
//					SlotAssignment<T> slot = assignments.ElementAt(slotNumber);
...
//	}
//
//	static class CostAndSlot<T : Vector<T>> implements Comparable<CostAndSlot<T>> {
//		float cost;
//		int slotNumber;
//
...
//	}
//
//	static class MemberAndSlots<T : Vector<T>> implements Comparable<MemberAndSlots<T>> {
...
//	}
//
//	public interface SlotCostProvider<T> where T : Vector<T> {
//
