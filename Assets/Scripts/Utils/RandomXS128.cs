 ...
 * @author davebaol */
    public class RandomXS128
    {
        ...
        public long nextLong()
        {
            ...
            this.seed0 = s0;
            s1 ^= s1 << 23;
            return (this.seed1 = (s1 ^ s0 ^ UnsignedRightShift(s1, 17) ^ UnsignedRightShift(s0, 26))) + s0;
        }

        public static int UnsignedRightShift(int v, int delta)
        {
            return (int)((uint)v >> delta);
        }

        public static long UnsignedRightShift(long v, int delta)
        {
            return (long)((ulong)v >> delta);
        }
        ...
        {
            if (n <= 0) throw new InvalidOperationException("n must be positive");
            for (; ; )
            {
                long bits = UnsignedRightShift(nextLong(), 1);
                long value = bits % n;
        ...
        {
            return UnsignedRightShift(nextLong(), 11) * NORM_DOUBLE;
        }
        ...
        {
            return (float)(UnsignedRightShift(nextLong(), 40) * NORM_FLOAT);
        }

        /** Returns a pseudo-random, uniformly distributed {@code bool } value from this random number generator's sequence.
         * <p>
         * This implementation uses {@link #nextLong()} internally. */
        ...
        {
            var y = (ulong)x;
            y ^= (ulong)UnsignedRightShift(x, 33);
            y *= 0xff51afd7ed558ccdL;
            y ^= (ulong)UnsignedRightShift(x, 33);
            y *= 0xc4ceb9fe1a85ec53L;
            y ^= (ulong)UnsignedRightShift(x, 33);
