 ...
 * 
 * @author davebaol */
public class RaycastObstacleAvoidance<T> : SteeringBehavior<T> where T : Vector<T>
{
    ...
    public RaycastObstacleAvoidance(Steerable<T> owner) : this(owner, null)
    {
    ...
    public RaycastObstacleAvoidance(Steerable<T> owner, RayConfiguration<T> rayConfiguration) : this(owner, rayConfiguration, null)
    {
    ...
    public RaycastObstacleAvoidance(Steerable<T> owner, RayConfiguration<T> rayConfiguration,
                                    RaycastCollisionDetector<T> raycastCollisionDetector) : this(owner, rayConfiguration, raycastCollisionDetector, 0)
    {
    ...
    public RaycastObstacleAvoidance(Steerable<T> owner, RayConfiguration<T> rayConfiguration,
                                    RaycastCollisionDetector<T> raycastCollisionDetector, float distanceFromBoundary) : base(owner)
    {
    ...
    protected override SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering)
    {
        ...
        float minDistanceSquare = float.PositiveInfinity;
        ...
        for (int i = 0; i < inputRays.Length; i++)
        {
            ...
            bool collided = raycastCollisionDetector.findCollision(outputCollision, inputRays[i]);
        ...
        if (minDistanceSquare == float.PositiveInfinity) return steering.setZero();
