 ...
 * 
 * @author davebaol */
public class PrioritySteering<T> : SteeringBehavior<T> where T: Vector<T> {
	...
	protected List<SteeringBehavior<T>> behaviors = new List<SteeringBehavior<T>>();
	...
	public PrioritySteering(Steerable<T> owner) : this(owner, 0.001f)
    {
	 ...
	 *           given no output */
	public PrioritySteering(Steerable<T> owner, float epsilon): base(owner)
    {
	...
	protected override SteeringAcceleration<T> calculateRealSteering (SteeringAcceleration<T> steering) {
		...
		for (int i = 0; i < n; i++) {
			...
			SteeringBehavior<T> behavior = behaviors.ElementAt(i);
