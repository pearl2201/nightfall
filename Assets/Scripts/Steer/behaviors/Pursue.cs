 ...
 *
 * @author davebaol */
public class Pursue<T> : SteeringBehavior<T> where T : Vector<T>
{
    ...
    public Pursue(Steerable<T> owner, Steerable<T> target) : this(owner, target, 1)
    {
     ...
     *           velocity. */
    public Pursue(Steerable<T> owner, Steerable<T> target, float maxPredictionTime) : base(owner)
    {
    ...
    protected virtual float getActualMaxLinearAcceleration()
    {
    ...
    public virtual Pursue<T> setTarget(Steerable<T> target)
    {
        this.target = target;
        return this;
    }
