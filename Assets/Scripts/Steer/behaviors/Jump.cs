 ...
 * informed through the {@link JumpCallback#reportAchievability(bool) reportAchievability} method. Also, if the jump is
 ...
 * 
 * @author davebaol */
public class Jump<T> : MatchVelocity<T> where T : Vector<T>
{
    ...
    public Jump(Steerable<T> owner, JumpDescriptor<T> jumpDescriptor, T gravity,
                GravityComponentHandler<T> gravityComponentHandler, JumpCallback callback) : base(owner)
    {
    ...
    protected override SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering)
    {
        ...
        return base.calculateRealSteering(steering);
    }
    ...
    private bool checkAirborneTimeAndCalculateVelocity(T outVelocity, float time, JumpDescriptor<T> jumpDescriptor,
    ...
    private class JumpTarget<T> : SteerableAdapter<T> where T : Vector<T>
    {

        public T position;
        public T linearVelocity;
        ...
        public override T getPosition()
        {
            return position;
        }


        public override T getLinearVelocity()
        {
	 ...
	 * 
	 * @author davebaol */
    public class JumpDescriptor<T> where T : Vector<T>
    {
	 ...
	 * 
	 * @author davebaol */
    public interface GravityComponentHandler<T> where T : Vector<T>
    {
    ...
    public interface JumpCallback
    {
        ...
        void reportAchievability(bool achievable);
