 ...
 * 
 * @author davebaol */
public class BlendedSteering<T> : SteeringBehavior<T> where T : Vector<T>
{
	...
	public BlendedSteering(Steerable<T> owner) : base(owner)
	{


		this.list = new List<BehaviorAndWeight<T>>();
	...
	protected override SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> blendedSteering)
	{
		...
		for (int i = 0; i < len; i++)
		{
			BehaviorAndWeight<T> bw = list.ElementAt(i);
	...
	public class BehaviorAndWeight<T> where T : Vector<T>
	{

		public SteeringBehavior<T> behavior;
		public float weight;
