 ...
 * 
 * @author davebaol */
public class Interpose<T> : Arrive<T> where T : Vector<T>{
	...
	public Interpose(Steerable<T> owner, Steerable<T> agentA, Steerable<T> agentB):this(owner, agentA, agentB, 0.5f) {
	...
	public Interpose(Steerable<T> owner, Steerable<T> agentA, Steerable<T> agentB, float interpositionRatio) :base(owner){
