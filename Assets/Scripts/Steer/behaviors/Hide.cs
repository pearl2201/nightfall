 ...
 *
 * @author davebaol */
public class Hide<T> : Arrive<T>, ProximityCallback<T> where T : Vector<T>
{
	...
	public Hide(Steerable<T> owner):	this(owner, null)
	{
	...
	public Hide(Steerable<T> owner, Location<T> target):	this(owner, target, null)
	{
	...
	public Hide(Steerable<T> owner, Location<T> target, Proximity<T> proximity):base(owner, target)
	{
		
		this.proximity = proximity;

		this.bestHidingSpot = newVector(owner);
		this.toObstacle = default; // Set to null since we'll reuse steering.linear for this vector
	}
