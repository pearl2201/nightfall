 ...
 * 
 * @author davebaol */
public class FollowPath<T, P> : Arrive<T> where T : Vector<T> where P : PathParam
{
    ...
    public FollowPath(Steerable<T> owner, Path<T, P> path) : this(owner, path, 0)
    {
	 ...
	 *           reverse direction. */
    public FollowPath(Steerable<T> owner, Path<T, P> path, float pathOffset) : this(owner, path, pathOffset, 0)
    {
    ...
    public FollowPath(Steerable<T> owner, Path<T, P> path, float pathOffset, float predictionTime): base(owner)
    {
        this.path = path;
	 ...
	 * @return this behavior for chaining. */
    public FollowPath<T, P> setArriveEnabled(bool arriveEnabled)
    {
