 ...
 * 
 * @author davebaol */
public class InfiniteProximity<T> : ProximityBase<T> where T : Vector<T>
{

    /** Creates a {@code InfiniteProximity} for the specified owner and list of agents.
	 * @param owner the owner of this proximity
	 * @param agents the list of agents */
    public InfiniteProximity(Steerable<T> owner, IEnumerable<Steerable<T>> agents) : base(owner, agents)
    {
        
    }


    public override int findNeighbors(ProximityCallback<T> callback)
    {
        int neighborCount = 0;
        foreach (Steerable<T> currentAgent in agents)
        {
