 ...
 * @fix JiangZhiYong
 */
public class MessageDispatcher : Telegraph
{


    private MemoryPool<Telegram> POOL = new MemoryPool<Telegram>(16);

    private PriorityQueue<Telegram> queue;

    private Dictionary<int, List<Telegraph>> msgListeners; // 消息监听器

    private Dictionary<int, List<TelegramProvider>> msgProviders;

    /** Creates a {@code MessageDispatcher} */
    public MessageDispatcher()
    {
        this.queue = new PriorityQueue<Telegram>();
        this.msgListeners = new Dictionary<int, List<Telegraph>>();
        this.msgProviders = new Dictionary<int, List<TelegramProvider>>();
    }
    ...
    {
        List<Telegraph> listeners = msgListeners[msg];
        if (listeners == null)
        {
            listeners = new List<Telegraph>(16);
            msgListeners.Add(msg, listeners);
        }
        listeners.Add(listener);

        // Dispatch messages from registered providers
        List<TelegramProvider> providers = msgProviders[msg];
        if (providers != null)
        {
            for (int i = 0, n = providers.Count(); i < n; i++)
            {
                TelegramProvider provider = providers.ElementAt(i);
                object info = provider.provideMessageInfo(msg, listener);
                if (info != null)
                {
                    Telegraph sender = provider is Telegraph ? (Telegraph)provider : null;
	 ...
	 *            the message codes
	 */
    public void addListeners(Telegraph listener, params int[] msgs)
    {
    ...
    public void addProvider(TelegramProvider provider, int msg)
    {
        ...
        if (providers == null)
        {
            ...
            providers = new List<TelegramProvider>(16);
            msgProviders.Add(msg, providers);
        }
	 ...
	 *            the message codes
	 */
    public void addProviders(TelegramProvider provider, params int[] msgs)
    {
        foreach (int msg in msgs)
	 ...
	 *            the message codes
	 */
    public void removeListener(Telegraph listener, params int[] msgs)
    {
        foreach (int msg in msgs)
	 ...
	 *            the message codes
	 */
    public void clearListeners(params int[] msgs)
    {
        foreach (int msg in msgs)
            clearListeners(msg);
    }
	 ...
	 *            the message codes
	 */
    public void clearProviders(params int[] msgs)
    {
        foreach (int msg in msgs)
            clearProviders(msg);
    }
	 ...
	 */
    public void dispatchMessage(Telegraph sender, int msg, bool needsReturnReceipt)
    {
	 ...
	 *             if the sender is {@code null} and the return receipt is needed
	 */
    public void dispatchMessage(Telegraph sender, int msg, object extraInfo, bool needsReturnReceipt)
    {
	 ...
	 *             if the sender is {@code null} and the return receipt is needed
	 */
    public void dispatchMessage(Telegraph sender, Telegraph receiver, int msg, bool needsReturnReceipt)
    {
	 ...
	 *             if the sender is {@code null} and the return receipt is needed
	 */
    public void dispatchMessage(int delay, Telegraph sender, int msg, bool needsReturnReceipt)
    {
	 ...
	 *             if the sender is {@code null} and the return receipt is needed
	 */
    public void dispatchMessage(int delay, Telegraph sender, int msg, object extraInfo, bool needsReturnReceipt)
    {
	 ...
	 *             if the sender is {@code null} and the return receipt is needed
	 */
    public void dispatchMessage(int delay, Telegraph sender, Telegraph receiver, int msg, bool needsReturnReceipt)
    {
            ...
            bool needsReturnReceipt)
    {
        if (sender == null && needsReturnReceipt)
            throw new Exception("Sender cannot be null when a return receipt is needed");

        // Get a telegram from the pool
        Telegram telegram = POOL.get<Telegram>();
        ...
        else
        {
            ...
            telegram.setTimestamp(DateTime.UtcNow.Millisecond + delay);
            ...
            {
                POOL.put(telegram);
                PikaLogger.Debug("Delayed telegram from " + sender + " for " + receiver
    ...
    {
        float currentTime = DateTime.UtcNow.Millisecond;
        ...
        {
            Telegram telegram = queue.ElementAt(i);
            callback.report(telegram.getTimestamp() - currentTime, telegram.sender, telegram.receiver, telegram.message,
                    telegram.extraInfo, telegram.returnReceiptStatus);
        }
    }
    ...
    private void discharge(Telegram telegram)
    {
        if (telegram.receiver != null)
        {
            ...
            if (!telegram.receiver.handleMessage(telegram))
            {
                ...
                PikaLogger.Debug("消息{}未处理", telegram.message);
            }
        }
        else
        {
            ...
            int handledCount = 0;
            List<Telegraph> listeners = msgListeners[telegram.message];
            if (listeners != null)
            {
                for (int i = 0; i < listeners.Count(); i++)
                {
                    if (listeners.ElementAt(i).handleMessage(telegram))
                    {
                        handledCount++;
                    }
                }
