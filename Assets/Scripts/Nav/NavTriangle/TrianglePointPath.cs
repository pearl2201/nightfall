 ...
 * @fix JiangZhiYong
 */
public class TrianglePointPath : IEnumerable<Vector3>
{
    ...
    private class Funnel
    {
        ...
        {
            leftPlane.set(pivot, new Vector3().set(pivot).add(V3_UP), leftEdgeVertex);
        ...
        {
            rightPlane.set(pivot, new Vector3().set(pivot).add(V3_UP), rightEdgeVertex); // 高度
    ...
    {
        return (TriangleEdge)((index == nodes.Count()) ? lastEdge : nodes.ElementAt(index));
    }
    ...
    public void calculateForGraphPath(TriangleGraphPath trianglePath, bool calculateCrossPoint)
    {
        ...
        {
            lastEdge = new TriangleEdge(nodes.ElementAt(nodes.Count() - 1).getToNode(), nodes.ElementAt(nodes.Count() - 1).getToNode(), end,
                    end);
