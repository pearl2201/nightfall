...
namespace MasterServerToolkit.MasterServer
{
    public abstract class ServerBehaviour : MonoBehaviour, IServer
    {
        ...
        protected virtual void Start()
        {
            ...
            socket.UseSecure = Mst.Settings.UseSecure ? Mst.Settings.UseSecure : useSecure;
            ...
            socket.SslProtocols = Mst.Settings.SslProtocols = sslProtocols;
        ...
        protected virtual void OnMessageReceived(IIncomingMessage message)
        {
            try
            {
                ...
                if (handler == null)
                {
                    logger.Error($"You are trying to handle message with OpCode [{message.OpCode}]. " +
            ...
            catch (Exception e)
            {
                ...
                logger.Error($"An error occurred while handling a message from client. Message OpCode: [{message.OpCode}], Error: {e}");
